...
module.exports = {
  ...
  // 搜尋功能 - 使用 Algolia DocSearch (推薦) 或本地搜尋
  // 選項 1: Algolia DocSearch (生產環境推薦)
  themeConfig: {
    algolia: {
      appId: 'YOUR_APP_ID',
...
**本地環境設計：**
- 本地開發環境是建立在 VM 中，因此若建立網頁伺服器要測試，不是 `localhost` ，而是用 `localhost`

**自訂首頁設計：**
- 根據 new-index.html 設計範本，實作具有橘色漸層主視覺的首頁，主題色為 #F05E1C
- 首頁包含 Hero 區塊展示「Ycat 的個人成長知識庫」標題
- 整合最新文章列表，自動顯示前五篇最新部落格文章
- 採用響應式設計，支援桌面和行動裝置
- 使用自訂 CSS 樣式，包含動畫效果和漸層背景
